...
def _build_system_prompt(knowledge: str, assistant_style: str) -> str:
    base = (
        ...
        "Отвечай по фактам из переданного контекста и не выдумывай лишнего. "
        "Если данных недостаточно, скажи это в голосе Кейна кратко и спокойно. "
...
def _build_knowledge_context(question: str, section_key: str | None, topic_key: str | None) -> str:
    ...
    knowledge = get_prompt_knowledge(selected_ids)
    if knowledge:
        return knowledge
    return get_prompt_knowledge(["0"])
...
def _select_block_ids(question: str, section_key: str | None, topic_key: str | None) -> list[str]:
    ...
    if any(word in lowered for word in ("принцесс", "принц", "альфред", "тэо", "присцилл", "леймарис")):
        selected.append("3")
    if any(word in lowered for word in ("лир", "граал", "вибраниум", "ресурс", "дуэл", "сундук", "шкала", "ретранслятор", "купол", "бонус")):
        selected.append("5")
    if any(word in lowered for word in ("риммэль", "валенти", "эпох", "легион", "истори", "город", "королевств", "купол", "ретранслятор")):
        selected.append("0")
